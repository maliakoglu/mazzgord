...
export default function AccountScreen() {
  ...
  return <ScreenContainer edges={["top", "left", "right"]}><ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ padding: 20, paddingBottom: 35 }}>
    ...
    {(orders?.quotes || []).slice(0, 5).map((q) => <Pressable key={q.id} onPress={() => router.push({ pathname: "/(tabs)/offer-detail", params: { id: String(q.id) } })} style={{ backgroundColor: colors.surface, borderRadius: 18, borderWidth: 1, borderColor: colors.border, padding: 16, marginBottom: 11 }}><View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}><View><Text style={{ color: colors.foreground, fontSize: 15, fontWeight: "800" }}>MZ-{String(q.id).padStart(5, "0")}</Text><Text style={{ color: colors.muted, fontSize: 12, marginTop: 4 }}>{q.source_language} → {q.target_language} · {formatDate(q.created_at)}</Text></View><View style={{ flexDirection: "row", gap: 6 }}>{q.offer_status === "offered" && <Badge tone="orange">Teklif</Badge>}{q.offer_status === "accepted" && <Badge tone="green">Kabul</Badge>}<Badge>{STATUS_LABELS[q.order_status] || q.order_status}</Badge></View></View></Pressable>)}
