  ...
  return (
    <section id="portfolio" className="py-20 md:py-32 parallax-yorumlar" style={{ backgroundColor: 'var(--color-soft-sand)' }}>
      <div className="container mx-auto px-4" style={{ maxWidth: '1200px' }}>
        <h2 className="text-center mb-4 parallax-section" style={{ fontFamily: '"Playfair Display", serif', fontSize: '36px', fontWeight: 700, color: 'var(--color-ink-black)', letterSpacing: '-0.02em' }}>Başarılı Projeler</h2>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {[
            ...
            <div key={idx} className="p-8 transition" style={{ backgroundColor: 'var(--color-paper-white)', borderRadius: '24px', boxShadow: 'rgba(213, 208, 184, 0.4) 0px 2px 6px 0px' }}
              ...
              <h3 style={{ fontFamily: '"Playfair Display", serif', fontSize: '24px', fontWeight: 500, marginBottom: '12px', color: 'var(--color-ink-black)' }}>{project.title}</h3>
              <p className="mb-4" style={{ color: 'var(--color-mid-stone)', fontSize: '15px', lineHeight: 1.63 }}>{project.description}</p>
              <div className="flex gap-2 flex-wrap">
                {project.tags.map((tag, tagIdx) => (
                  <span key={tagIdx} className="tag tag-teal">{tag}</span>
                ))}
              </div>
            </div>
          ))}
