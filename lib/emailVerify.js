...
// 6 haneli kriptografik güvenli kod
...
// Doğrulama sonrası işaretle (30 dakika geçerli)
export async function markEmailVerified(env, email) {
  const key = `verified:${email.toLowerCase()}`;
  await env.RATE_LIMIT.put(key, "1", { expirationTtl: 1800 });
}
